...
# Phrases that signal someone is asking for info already shared / unclear
CLARIFY_PATTERNS = [
    r"\?",
    r"^/(ask|catchup|decisions|tasks|meeting|find|changed|help)\b",
    r"\bwhat (did|was|is|are|about|happened|happend|happen)\b",
    r"\bwho (said|decided|suggested|proposed|is)\b",
...
def needs_clarification(text: str, *, is_group: bool = False, was_mentioned: bool = False) -> bool:
    ...
    exist in shared group / meeting memory, or when the bot is directly mentioned.
    """
    raw = (text or "").strip()
    ...
    # Check for question/clarification patterns
    if any(p.search(raw) for p in _COMPILED):
        return True

    if not is_group:
        # Direct chats: also treat slash-less natural asks
        lower = raw.lower()
        if lower.startswith(("ask ", "tell me", "explain", "help")):
            return True
